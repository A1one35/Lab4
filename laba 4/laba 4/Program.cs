...
namespace laba_4
{
    class Program
    {
        ...
        {
            string[] directiry = Directory.GetFiles(@"E:\textLaba4");
            ...
            for (int i = 0; i < directiry.Length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        using (StreamWriter sw = new StreamWriter(@"E:\textLaba4\no_file.txt", true, System.Text.Encoding.Default))
                        {
        ...
        public static int[] ReadAllInf(string directiry)
        {
            ...
            if (inform.Length > 2)
            {
                using (StreamWriter sw = new StreamWriter(@"E:\textLaba4\bad_data.txt", true, System.Text.Encoding.Default))
                {
                    sw.Write(directiry + "\n");
                    return array;
                }
        ...
        public static int TryFindProduct(int[] array, string directiry)
        {
            ...
            {
                using (StreamWriter sw = new StreamWriter(@"E:\textLaba4\overflow.txt", true, System.Text.Encoding.Default))
                {
                    sw.Write(directiry + "\n");
                }
            }
